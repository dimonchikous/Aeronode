...
3. (Необязательно) Отконфигурируйте файлы `lib/AeronodeConfig/AeronodeConfig.h` и `lib/AeroDisplay/AeroDisplay.h`
    * **Программная коррекция** обозначена в блоке *Software correction*, изначально везде стоит значение 0.
...
5. Запустив устройство впервые, подключитесь к точке доступа, которое оно создало для настройки.
    * **Обратите внимание!**. Во время настройки(указания ip-адреса сервера) у вас попросят указать **порт**, порт бота по умолчанию **5000**. Чтобы настроить порт бота, в самом конце `server/main.py` измените значение `port` на вам нужное.
